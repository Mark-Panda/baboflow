...
type ArcheryQueryNodeConfiguration struct {
	// ConnectionID 引用的 archery_connection 的 ID（凭据加密存库，节点只存引用）。
	ConnectionID int64 `json:"connectionId" label:"Archery连接" desc:"要查询的 Archery 连接（在连接管理中配置）" required:"true"`
...
func (n *ArcheryQueryNode) Init(_ types.Config, configuration types.Configuration) error {
	...
	if n.config.ConnectionID <= 0 {
		return errors.New("archeryQuery 节点缺少必填配置 connectionId")
	}
