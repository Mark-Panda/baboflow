...
import { App, Breadcrumb, Button, Input, Space, Spin, Tag, Tooltip } from 'antd';
import {
  ArrowLeftOutlined, SaveOutlined, CloudUploadOutlined, CaretRightOutlined,
  ApartmentOutlined, CheckCircleFilled,
...
import { dslToFlow, flowToDsl, layoutFlow, isContainerType, RuleNodeData, DslChain } from './chainDsl';
...
export default function ChainEditorPage() {
  ...
  const [stack, setStack] = useState<Frame[]>([{ nodeId: null, label: '根', nodes: [], edges: [] }]);
  ...
  const { data: compData } = useQuery({ queryKey: ['components'], queryFn: () => componentApi.list(), staleTime: 5 * 60 * 1000 });
  const components = useMemo(() => compData?.list ?? [], [compData]);

  const cur = stack[stack.length - 1];
  const setCur = useCallback(
    (patch: Partial<Frame>) => {
      setStack((st) => st.map((f, i) => (i === st.length - 1 ? { ...f, ...patch } : f)));
      setDirty(true);
    },
    []
  );
  ...
  const enterSub = useCallback(
    (nodeId: string) => {
      ...
      if (!node || !isContainerType((node.data as RuleNodeData).ruleType)) return;
      const d = node.data as RuleNodeData;
      const sub = d.subFlow ?? { nodes: [], edges: [] };
      setStack((st) => [...st, { nodeId, label: d.name, nodes: sub.nodes, edges: sub.edges }]);
      ...
      setStack((st) => {
        ...
        for (let i = next.length - 1; i > index; i--) {
          ...
          parent.nodes = parent.nodes.map((n) =>
            n.id === popped.nodeId
              ? { ...n, data: { ...n.data, subFlow: { nodes: popped.nodes, edges: popped.edges } } }
              : n
          );
          next.splice(i, 1);
        }
  ...
  const collapsedRoot = useCallback((): Frame => {
    const next = stack.map((f) => ({ ...f, nodes: [...f.nodes], edges: [...f.edges] }));
  ...
  const onNodeDataChange = useCallback(
    (nodeId: string, patch: Partial<RuleNodeData>) => {
      setCur({
        nodes: cur.nodes.map((n) => (n.id === nodeId ? { ...n, data: { ...n.data, ...patch } } : n)),
      });
    },
    [cur.nodes, setCur]
  );
  ...
  const buildDsl = useCallback((): DslChain => {
    ...
    return flowToDsl({ id: chainId || undefined, name }, root.nodes, root.edges);
  ...
  const onDebug = useCallback(
    async (input: string) => {
      ...
      setLastRun(null);
      try {
        const res = await chainApi.debug(chainId, { data: input, dataType: 'JSON' });
        setLastRun({ output: res.output, error: res.error, traces: res.nodeTrace ?? [] });
        // 逐节点着色
    ...
    <ReactFlowProvider>
      <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#f5f6fa' }}>
        ...
        <div className="bf-editor-bar">
          <Tooltip title="返回列表">
            <Button type="text" icon={<ArrowLeftOutlined />} onClick={() => navigate('/chains')} />
          </Tooltip>
          <Input
            value={name}
            ...
            placeholder="规则链名称"
          />
          <Tag color={status === 'published' ? 'green' : 'default'} style={{ marginLeft: 4 }}>
            {status === 'published' ? `已发布 v${version}` : '草稿'}
          </Tag>
          {dirty && <span style={{ color: '#e6a23c', fontSize: 12 }}>●未保存</span>}
          {hasStates && !dirty && (
          ...
          <Space style={{ marginLeft: 'auto' }} size="small">
            ...
            <Button type="primary" icon={<CaretRightOutlined />} loading={running} onClick={() => onDebug('{}')}>
              调试
            </Button>
          </Space>
        </div>
        ...
        {/* 面包屑（子画布层级） */}
        {stack.length > 1 && (
          <div style={{ padding: '6px 14px', background: '#fbfcfe', borderBottom: '1px solid #eef0f5' }}>
            <Breadcrumb
              items={stack.map((f, i) => ({
                title: (
                  <a onClick={() => jumpTo(i)} style={{ color: i === stack.length - 1 ? '#1f2430' : '#4f6ef7' }}>
                    {i === 0 ? name || '根' : f.label}
                  </a>
                ),
              }))}
