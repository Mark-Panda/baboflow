...
// 分组顺序
const ORDER = ['endpoint', 'filter', 'transform', 'action', 'external', 'common', 'flow'];
  ...
  return (
    <div className="bf-palette">
      ...
      {isLoading ? (
        ...
        <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="无匹配组件" style={{ marginTop: 30 }} />
      ) : (
        <Collapse
          ghost
          size="small"
          defaultActiveKey={groups.map(([c]) => c)}
          items={groups.map(([cat, items]) => ({
            key: cat,
            label: (
              <span style={{ fontSize: 12, fontWeight: 600, color: catStyle(cat).color }}>
            ...
            children: items.map((c) => (
              <div
                ...
                style={{ ['--cat-color' as string]: catStyle(c.category).color }}
                draggable
